...
namespace Play.Trading.Service.StateMachines
{
    public class PurchaseStateMachine : MassTransitStateMachine<PurchaseState>
    {
        ...
        private void ConfigureEvents()
        {
            ...
            Event(() => GilDebited);
            Event(() => GrantItemsFaulted, e => e.CorrelateById(
                context => context.Message.Message.CorrelationId));
            Event(() => DebitGilFaulted, e => e.CorrelateById(
                context => context.Message.Message.CorrelationId));
        }
        ...
        private void ConfigureInitialState()
        {
            Initially(
                When(PurchaseRequested)
                    .Then(context =>
                    {
                        // Captured the initial data into store machine instance
                        ...
                        purchaseStartedCounter.Add(
                            1, 
                            new KeyValuePair<string, object>(nameof(context.Saga.ItemId), context.Saga.ItemId)
                        );
                    })
                    // Calculate the Purchase Total
                    .Activity(x => x.OfType<CalculatePurchaseTotalActivity>())
                    //// Immediately send the Grant Items command to the Inventory Micro service
                    //// Then, Inventory service emit InventoryItemsGranted event.
